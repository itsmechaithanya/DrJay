...
function Publications() {
    ...
    const categoryFilters = ['Show all', 'AI & Ethics', 'HCI', 'FinTech', 'Education'];
    ...
    return (
        <div className="w-screen min-h-screen bg-[#0a0a0a] pt-[15vh] md:pt-[25vh] px-[6vw] md:px-[5vw] text-white">
            ...
            <div className="flex flex-col gap-6 mb-[6vh] md:mb-[8vh]">
                <div className="flex flex-col md:flex-row md:items-center gap-3">
                    <span className="text-[#a0a0a0] text-xs font-semibold uppercase tracking-wider min-w-[100px]">Category</span>
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        {categoryFilters.map(filter => (
                            <button
                                key={filter}
                                onClick={() => setActiveCategory(filter)}
                                className={`px-4 md:px-6 py-2 md:py-2.5 rounded-full md:rounded-4xl text-[0.85rem] md:text-[0.95rem] font-medium transition-all duration-300 ${activeCategory === filter
                                    ? 'bg-white text-black'
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-3">
                    <span className="text-[#a0a0a0] text-xs font-semibold uppercase tracking-wider min-w-[100px]">Year</span>
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        {yearFilters.map(year => (
                            <button
                                key={year}
                                onClick={() => setActiveYear(year)}
                                className={`px-4 md:px-6 py-2 md:py-2.5 rounded-full md:rounded-4xl text-[0.85rem] md:text-[0.95rem] font-medium transition-all duration-300 ${activeYear === year
                                    ? 'bg-white text-black'
                                    : 'bg-[#1c1c1c] text-[#a0a0a0] hover:bg-[#2a2a2a]'
                                    }`}
                            >
            ...
            <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 md:gap-x-14 gap-y-12 md:gap-y-18 pb-[10vh] md:pb-[15vh]">
                {publications.filter(pub => (activeCategory === 'Show all' || pub.category === activeCategory) && (activeYear === 'Show all' || pub.year === activeYear)).map(pub => (
                ...
                {publications.filter(pub => (activeCategory === 'Show all' || pub.category === activeCategory) && (activeYear === 'Show all' || pub.year === activeYear)).length === 0 && (
                    <div className="col-span-full py-[10vh] text-center text-[#a0a0a0] text-xl">
                        No publications found for the selected category and year.
                    </div>
                )}
            </div>
        </div>
